    ...
    for j=1:length(ReM_OcM_Ratio)
        for k=1:length(REM)
            for l=1:length(PV_gen_share)
                Output = CombinedCases2050(PV_gen_share(l), ReM_OcM_Ratio(j), REM(k), Storage_hrs(i), SolarData, WindData, HourlyElectricityPrices);
