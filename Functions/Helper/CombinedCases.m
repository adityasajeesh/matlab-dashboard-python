function [Output] = CombinedCases(PV_gen_share, ReM_OcM_Ratio, ReM, Storage_hrs, SolarData, WindData, HourlyElectricityPrices)
%% CombinedCases
%   Description:
%       Function to take the optimisation parameters and model data to
%       call various functions to perform various calculations
%
%   Input: 
%       PV_gen_share: Percentage of demanded power covered by the solar farm
%       ReM_OcM_Ratio: A ratio to calculate the overcapacity multiple from the allocated renewable multiple
%       ReM: Renewable multiple - a multiple established to determine oversizing of renewable farms
%       Storage_hrs: A measure of hydrogen storage, 1 storage hour of storage should last for 1 hour without additional storage added
%       SolarData: An array consisting of solar irradiance data for a year
%       WindData: An array consisting of wind speed data for a year
%       HourlyElectricityPrices: An array consisting of day ahead electricity prices at each hour for a year
%
%   Output: 
%       Output: Output struct containing important variables calculated throughout this function

%% Establish case variables
LC. PV_gen_share   = PV_gen_share  ; % Percentage of solar wind splits
LC. ReM            = ReM           ; % Renewable Multiple
LC. ReM_OcM_Ratio  = ReM_OcM_Ratio ;
LC. Storage_hrs    = Storage_hrs   ; % hrs

%% Struct Variables from Mass Balance Excel Sheet
LC. OcM = LC.ReM_OcM_Ratio * LC.ReM;
LC. HourlyDemandedHydrogen = 8546.86487 + 88380/49.9; %kg/hr
...
% The amount of renewable hydrogen produced from the electrolysers (not including hydrogen produced from obtained backup energy)
...
% The amount of hydrogen produced from the electrolysers (renewable and backup power included)
LC.HourlyProducedHydrogen = ElectStorageOutput.ElectrolyserInputPower/LC.GH_Spec_Cons;

% Annual useful energy produced
LC.Ann_energy_prod = sum(TotalPower) - sum(ElectStorageOutput.Curtailment); %kW

% The total price of backup power spent throughout the year for the day ahead prices scenario
SumElectPrice = ElectStorageOutput.BackUpElectricity./1000 .* reshape(HourlyElectricityPrices, 1, 8784);
LC.SumElectPrice = sum(SumElectPrice);

% The total price of backup power spent throughout the year for a power purchase agreement scenario
ElectPricePPA = ElectStorageOutput.BackUpElectricity./1000 * 97.5; % $97.5 AUD
...
Output.ProducedRenewableHydrogen = LC.HourlyProducedRenewableHydrogen; % does not include backup
Output.ProducedHydrogen = LC.HourlyProducedHydrogen; % does include backup
...
Output.DayAheadElectricityPrice = LC.SumElectPrice/1000000;
Output.PPAElectricityPrice = LC.ElectPricePPA/1000000;

Output.TotalMassOutput = ElectStorageOutput.NumElectrolysers * LC.MassOutput / 24;
...
Output.GreenHydrogen_LCOH_Reporting_All_PPA = GreenHydrogenOutput.LCOH_Reporting_All_PPA;
...
Output.GreenHydrogen_LCOS_Reporting_All_PPA = GreenHydrogenOutput.LCOS_Reporting_All_PPA;
